...
1. **Fraud Inquiry Costs** (primary — matches the existing FGP monthly invoice sheet). One row per FI, sorted alphabetically. Columns in this exact order:
   ...
   | :-------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | **A**     | `Client` (= `fi_name`)                                                                                                                                                                                                                                                                                                  |
   | **B**     | `FGP Qty` (= `max(effectiv_calls, plaid_base)` — i.e. `max(M04-FRA Qty, IV-Base Qty)`; M04-FRA is the Socure call proxy, IV-Base is the Plaid call proxy)                                                                                                                                                               |
   ...
   Bold header row, freeze panes at column B / row 2, currency formatting (`$#,##0.00`) on every Cost column, header with two sub-rows showing the unit price under each column header (replicate the source sheet's header layout: row 1 vendor group label, row 2 line-item, row 3 "Qty" / "Cost ($X)" with the price). Append a bold **TOTAL (Our Clients)** row. For the TOTAL row, sum all Qty / Cost columns from `totals.costs` EXCEPT `FGP Qty` — recompute the TOTAL's FGP Qty as `max(totals.effectiv_calls, totals.plaid_base)` (consistent with the per-FI formula). Demo FIs (`is_demo: true`) go at the bottom with name suffixed " (DEMO — excluded)" and totals row should EXCLUDE them.
...
3. **Methodology** — a text tab so finance can audit. Include verbatim:
   ...
   - The **FGP Qty formula**: `max(M04-FRA Qty, IV-Base Qty)` — i.e. the higher of the two vendor call totals per FI. Rationale: vendor totals are exact and auditable against vendor invoices; the per-applicant deduped count (`billable_inquiries` in the Summary tab) is still reported for reference but is a fuzzy match (~99% cross-vendor confidence). `max` is the conservative billing proxy when the same applicant runs both vendors.
