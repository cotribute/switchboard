...
1. **Fraud Inquiry Costs** (primary — matches the existing FGP monthly invoice sheet). One row per FI, sorted alphabetically. Columns in this exact order:
   ...
   | :-------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | **A**     | `Client` (= `fi_name`)                                                                                                                                                                                                                                                                                                  |
   | **B**     | `FGP Qty` (= `effectiv_calls + plaid_only` — every Effectiv eval counts (exact) plus the deduped count of applicants who ran only on Plaid; equals `billable_inquiries`)                                                                                                                                                |
   | **C–P**   | 7 Socure modules. Each has a Qty + Cost pair. **Qty for every module = `effectiv_calls`** (each Effectiv eval triggers all 7 modules). Cost = Qty × per-module unit price from `pricing.socure`: M01-EML (.0317), M02-PHO (.0317), M03-ADR (.0277), M04-FRA (.1584), M05-KYC (.1584), M06-WL1 (.0396), M17-SYN (.0554). |
   | **Q**     | `Socure Subtotal $` (= `costs.socure_subtotal`)                                                                                                                                                                                                                                                                         |
   | **R–S**   | `IV-Base Qty` (= `plaid_base`), `IV-Base Cost` (= `costs.plaid_base_cost`)                                                                                                                                                                                                                                              |
   | **T–U**   | `IV-Doc Qty` (= `plaid_doc`), `IV-Doc Cost` (= `costs.plaid_doc_cost`)                                                                                                                                                                                                                                                  |
   | **V–W**   | `IV-Selfie Qty` (= `plaid_selfie`), `IV-Selfie Cost` (= `costs.plaid_selfie_cost`)                                                                                                                                                                                                                                      |
   | **X–Y**   | `IV-Lightning Qty` (= `plaid_lightning`), `IV-Lightning Cost` (= `costs.plaid_lightning_cost`)                                                                                                                                                                                                                          |
   | **Z**     | `Plaid Subtotal $` (= `costs.plaid_subtotal`)                                                                                                                                                                                                                                                                           |
   | **AA**    | `Grand Total $` (= `costs.grand_total`)                                                                                                                                                                                                                                                                                 |

   Bold header row, freeze panes at column B / row 2, currency formatting (`$#,##0.00`) on every Cost column, header with two sub-rows showing the unit price under each column header (replicate the source sheet's header layout: row 1 vendor group label, row 2 line-item, row 3 "Qty" / "Cost ($X)" with the price). Append a bold **TOTAL (Our Clients)** row summing all Qty / Cost columns from `totals.costs` (including FGP Qty — the totals row is the simple sum of per-FI billable_inquiries values, which equals `totals.effectiv_calls + totals.plaid_only`). Demo FIs (`is_demo: true`) go at the bottom with name suffixed " (DEMO — excluded)" and totals row should EXCLUDE them.
...
3. **Methodology** — a text tab so finance can audit. Include verbatim:
   ...
   - The **FGP Qty formula**: `effectiv_calls + plaid_only` per FI — every Effectiv evaluation counts (exact, one per applicant on the smaller-volume side), plus the deduped count of applicants who ran only on Plaid (the "plaid_only" cohort from per-applicant name+DOB matching). Equals `billable_inquiries`, just written as exact-side + deduped-other-side for auditability. Rationale: matches the per-applicant contract while keeping the exact Effectiv count fully defensible; only the Plaid-only subset relies on fuzzy cross-vendor matching (~99% confidence). Equivalent to `plaid_base + effectiv_only`.
